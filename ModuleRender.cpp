...
// Blit to screen
bool ModuleRender::Blit(SDL_Texture* texture, int x, int y, SDL_Rect* section, float xScale, float yScale, float speed, double angle, int pivot_x, int pivot_y )
{
	bool ret = true;
	SDL_Rect rect;
	...
	if(section != NULL)
	{
		rect.w = section->w * xScale;
		rect.h = section->h * yScale;
	}
	else
	{
